...
type(interests_schema)

# COMMAND ----------

interests_schema.simpleString()
...
type(recent_purchases_schema)

# COMMAND ----------

raw_user_data_df.createOrReplaceTempView("rawuserdata")

# COMMAND ----------

# MAGIC %sql
# MAGIC select distinct(schema_of_json(recent_purchases) )from raw_user_data
...
display(parsed_users_df.select(
        "user_id", 
        explode("recent_purchases").alias("recent_purchase"))
    ) 
...
display(parsed_users_df.select(
        "user_id", 
        explode_outer("recent_purchases").alias("recent_purchase"))
    ) 
...
parsed_users_df.createTempView("parsed_user_data")
...
display(
(    parsed_users_df
        .selectExpr("1 as key", "explode(interests) as interest")
        .groupBy("key")
        .agg(
           collect_set("interest")
            .alias("collect_set(interest)"
               )
            ))
)
...
# MAGIC %sql
# MAGIC with base as (
# MAGIC   SELECT 1 as key, explode(interests) as interest from parsed_user_data
# MAGIC   )
# MAGIC select key , collect_list(interest) from base GROUP BY key
...
# MAGIC   SELECT 1 as key, explode(interests) as interest from parsed_user_data
# MAGIC   )
# MAGIC select key , collect_set(interest) from base GROUP BY key
...
recent_purchases_df.createTempView('recent_purchases')

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT collect_set( product_name) from recent_purchases
...
# MAGIC %sql
# MAGIC SELECT *
# MAGIC FROM recent_purchases
# MAGIC PIVOT (
# MAGIC   COUNT(product_id) AS quantity_purchased
# MAGIC   FOR product_name IN (
# MAGIC     -- List your product names here, e.g. 'ProductA', 'ProductB', 'ProductC'
# MAGIC     "External Monitor","Laptop","Mechanical Keyboard","Stylus Pen","Design Software","Graphics Tablet","Backpack","Textbook","Camera","Wireless Mouse","Financial Software","Educational Subscription","Notebook Set","Cooking Knives","Recipe Book","Legal Reference Book","Spice Set"
# MAGIC   )
# MAGIC )
