...
location_counts_gd = trips_df.groupBy("pickup_zip") 

# COMMAND ----------

type(location_counts_gd)

# COMMAND ----------

display(location_counts_gd.count().orderBy(desc("count")))
...
display(location_counts_gd.agg(
        count("*").alias("total_trips"),
        round(avg("trip_distance"), 2).alias("avg_distance"),
        round(avg("fare_amount"), 2).alias("avg_fare"),
        round(sum("fare_amount"), 2).alias("total_fare_amt")
...
print(type(window_by_trips))
print(type(window_by_fare))
...
window_by_trips_part = Window.partitionBy("pickup_zip").orderBy(desc("trip_distance"))
window_by_fare_part = Window.partitionBy("pickup_zip").orderBy(desc("fare_amount"))
...
# MAGIC library(dplyr)
# MAGIC trips_tbl <- sql("SELECT * FROM samples.nyctaxi.trips")
...
# MAGIC library(dplyr)
# MAGIC
# MAGIC result <- trips_tbl %>%
# MAGIC   group_by(pickup_zip) %>%
# MAGIC   arrange(trip_distance, .by_group = TRUE) %>%
# MAGIC   mutate(rn_by_distance = row_number()) %>%
# MAGIC   arrange(fare_amount, .by_group = TRUE) %>%
# MAGIC   mutate(rn_by_fare = row_number()) %>%
# MAGIC   ungroup() %>%
# MAGIC   select(
# MAGIC     pickup_zip,
# MAGIC     trip_distance,
# MAGIC     fare_amount,
# MAGIC     rn_by_distance,
# MAGIC     rn_by_fare
# MAGIC   )
# MAGIC
# MAGIC result %>% head()
...
# MAGIC
# MAGIC
# MAGIC spark_df_windowed <- trips_tbl %>%
# MAGIC   mutate(
# MAGIC     fare_rank = dense_rank() %>% 
# MAGIC       over(window_partition("pickup_zip") %>% 
# MAGIC              window_order("fare_amount"))
# MAGIC   )
# MAGIC                        
# MAGIC
...
# MAGIC library(dplyr)
# MAGIC
# MAGIC # Assuming 'sc' is your Spark connection and 'mtcars_tbl' is a Spark DataFrame
# MAGIC
# MAGIC # Define the window specification
# MAGIC my_window <- window_spec(
# MAGIC   partition_by = "pickup_zip",
# MAGIC   order_by = "fare_amount"
# MAGIC   # You can also add frame = c(-Inf, 0) for a cumulative window frame
# MAGIC )

# COMMAND ----------

# MAGIC %r
# MAGIC # Apply a window function using the defined specification
# MAGIC spark_df_windowed_spec = trips_tbl.select()%
# MAGIC   mutate(
# MAGIC     cumulative_mpg = over(
# MAGIC       x = dense_rank(fare_amount),
# MAGIC       window = my_window
# MAGIC     )
# MAGIC   )
