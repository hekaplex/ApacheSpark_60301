...
# MAGIC ## PLEASE READ!!!
# MAGIC
# MAGIC The above command will fail if you are using serverless. Please ensure that you are using Classic Compute and then re-run the notebook from the beginning.
...
null_counts_df = flights_temp_df.select(
    [
...
null_counts = null_counts_df.toPandas().iloc[0]

# Plot bar chart
plt.figure(figsize=(10, 6))
null_counts.plot.bar(color='skyblue')
plt.title('Null Counts per Column in flights_temp_df')
plt.xlabel('Column')
...
flights_required_cols_df.count()
...
flights_with_valid_data_df.count()

# COMMAND ----------

flights_with_valid_data_df.printSchema()
...
enriched_flights_df = enriched_flights_df.withColumn(
    "ElapsedTimeDiff", col("ActualElapsedTime") - col("CRSElapsedTime")
    ).drop("ActualElapsedTime", "CRSElapsedTime")

display(enriched_flights_df.select("FlightNum", "FlightDateTime", "ElapsedTimeDiff"))
